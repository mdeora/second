...
//            cell.messageModel = model;
            MessageModel *model = (MessageModel *)obj;
            if ([model isSender]) {
                if (self.as.user.avatarUrl == nil || [_as.user.avatarUrl isEqualToString:@""]) {
                    model.headImageURL = [[NSBundle mainBundle] URLForResource:@"chatListCellHead" withExtension:@"png"];
                }else{
                    model.headImageURL = [[NSURL alloc] initWithString:self.as.user.avatarUrl];
                }
                model.nickName = self.as.user.name;
            }else{
                model.headImageName = @"bei_ying.jpeg";
            }
            NSString *cellIdentifier = [EMChatViewCell cellIdentifierForMessageModel:model];
            EMChatViewCell *cell = (EMChatViewCell *)[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
            if (cell == nil) {
