...
-(void)initFriendDetailView{
    ...
    self.friendDetailView.backgroundColor = RGBACOLOR(255, 255, 255, 1);
     [self.friendDetailView renderUser:self.dataSource telNo:self.telPhone];
...
-(void)initAddFriendBtn{
    ...
    UIButton *addButton = [[UIButton alloc] initWithFrame:CGRectMake(30, addBtnY, VIEW_WEIGHT-60, 44)];
...
#warning 此处为添加好友
            ...
            NSDictionary *friendInfo = [self.dataSource objectAtIndex:0];
            NSLog(@">>>>>friendInfo>>>>>%@", friendInfo);
            NSDictionary *infoDic = [friendInfo objectForKey:@"return"];
            NSString *uid2 = [infoDic objectForKey:@"uid"];
            
            NSDictionary * postDict = [NSDictionary dictionaryWithObjectsAndKeys:uid2,@"uid2",[ManageVC sharedManage].uid,@"uid", nil];
