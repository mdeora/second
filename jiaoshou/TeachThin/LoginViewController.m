...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if([[NSUserDefaults standardUserDefaults]valueForKey:@"uid"]!=nil)
    {
        ...
        [[EaseMob sharedInstance].chatManager setApnsNickname:[ManageVC sharedManage].name];
...
-(void)setlayout
{
    ...
    imgv.image = [UIImage imageNamed:@"loginheader"];
    ...
    if([ManageVC sharedManage].LoginState==YES){
        tf1.text = [ManageVC sharedManage].name;
    }
    ...
    if([ManageVC sharedManage].LoginState==YES){
        tf1.text = [ManageVC sharedManage].pwd;
    }
