...
//        [MBHUDView dismissCurrentHUD];
        ...
        if ([code isEqualToString:@"01"]) {
           [self.contactsSource  addObjectsFromArray:[obj objectForKey:@"list"] ];
...
{
     NSLog(@">>>>>self.contactsSource.count>>>>%ld",self.contactsSource.count);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.realName = [buddyDic objectForKey:@"realname"];
   [cell renderFriendWithBuddyInfo:buddyDic];
...
#warning 会话者
            ChatViewController *chatVC = [[ChatViewController alloc] initWithChatter:self.realName isGroup:NO];
